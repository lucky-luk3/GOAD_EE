...
# ---------------------------------------------------------------------------
# REDIL2 variant : identical to REDIL, plus one minimal domain-joined member
# server (srv01) so the lab also shows sessions / AdminTo / CanRDP (lateral
...
SRV01 = {
    ...
    "domain": DOMAIN,
    "path": ou_dn("REDIL", "Admin", "Servers"),
    "use_laps": False,
    "local_groups": {
        # Server admins are local admins on servers (realistic) -> AdminTo edge.
        "Administrators": [f"{NETBIOS}\\Server Administrators"],
        # Helpdesk can RDP to the member server -> CanRDP edge.
        "Remote Desktop Users": [f"{NETBIOS}\\IT Support"],
    },
    "scripts": [],
...
if __name__ == "__main__":
    ...
    with open("/home/user/GOAD_EE/ad/REDIL2/data/config.json", "w",
              encoding="utf-8") as f:
        json.dump(config2, f, indent=4, ensure_ascii=False)
    print("REDIL2 -> same + member server srv01 (", SRV01["hostname"], ")")
